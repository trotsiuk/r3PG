...
mcmc_out <- runMCMC(
  ...
  sampler = "DEzs",
  settings = list(iterations = 4e+03, nrChains = 3, thin = 10))
```
...
As the first step, we can look at the summary of the outputs, which includes both estimated posterior mean values and convergence diagnostics (Gelman-Rubin potential scale reduction factors = psrf) for all parameters. For an MCMC chain to be converged, one typically requires the univariate psrf to be \< 1.05 and the multivariate psrf $\le$ 1.1 or 1.2, which is the case here.
