...
date: "`r Sys.Date()`"
abstract: "This vignette provides an overview of the r3PG R package functions and options. We provide a working example that (i) demonstrates the basic functionality and use of the package, (ii) performs a sensitivity analysis and a Bayesian calibration of the model, and (iii) uses the calibrated model to simulate spatial patterns of forest growth across Switzerland.  \n \n \n"
output: 
...
-   check the model input for consistency in structure
...
In this example, we run a simulation for mixed *Fagus sylvatica* and *Pinus sylvestris* stands (Forrester et al., 2017). The input data are provided as internal data in `r3PG`. You can inspect their structure by exploring, e.g., `d_mixture$site`.
...
As a second example, we will first perform a `Morris` screening and then perform the `Bayesian calibration` of the 3-PGpjs model. We will use the `Solling` forest flux site located in Germany at an elevation of 508 m.a.s.l., and dominated by *Picea abies*. We obtained data for this site via the [PROFOUND database](https://doi.org/10.5194/essd-2019-220) that was set up for evaluating vegetation models and simulating climate impacts on forests (Reyer et al., 2019). There are almost 50 years of records for various stand and flux variables for the `Solling` site.

### Likelihood function

A sensitivity analysis requires a target variable for which we want to evaluate sensitivity. In principle, any variable that is predicted by the model could be used as a target, including any of the variables that we plotted above. In this case, we are planning to run a Bayesian calibration later, and thus it makes sense to run the sensitivity on the difference between model predictions and data, which is measured by the likelihood function.
...
r3pg_sim <- function( par_df, ...){
  ...
  sim.df <- run_3PG(
    ...
    parameters = par_df, 
    size_dist = NULL,
    settings =  list(light_model = 1, transp_model = 1, phys_model = 1, mort_model = 1, 
      height_model = 1, crown_width_model = 1, calculate_d13c = 0),
    check_input = TRUE, ...)
...
To evaluate the sensitivity of the likelihood above to changes of the model parameters, we used Morris screening. Morris screening is a so-called global sensitivity method which quantifies parameter sensitivity in a defined area of the parameter space. This means we need to set sensible min / max values for all model parameters. Here, we provide expert-chosen values for all parameters, including the error parameters that are used in the likelihood.
...
As a next step, we want to calibrate the model. To speed up this task, we calibrate only the 20 most sensitive parameters identified in the `morris` screening plus the six error parameters for each of the observational variables.
...
We then simulate forest growth using the defaults and the drawn parameter combinations. Afterwards, we visualize the posteriori prediction range by drawing the $5^{th}$ and $95^{th}$ range of predictions. The calibrated model simulates the observational data well.
...
r3pg_grid <- function(site, forc){
  ...
  r3pg_int <- function( par_df){
    ...
    out <- run_3PG(
      ...
      parameters = par_df, 
      size_dist = NULL,
      list(light_model = 1, transp_model = 1, phys_model = 1, mort_model = 1, 
        height_model = 1, crown_width_model = 1, calculate_d13c = 0), 
      check_input = TRUE,
      df_out = F)[,,4,1]
