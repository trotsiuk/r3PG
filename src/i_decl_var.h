...
real(kind=kind(0.0d0)) :: st_Power     ! Power in self-thinning rule (mort_model = 2)
real(kind=kind(0.0d0)) :: st_Intercept ! Intercept for self-thinning rule (mort_model = 2)
real(kind=kind(0.0d0)) :: st_fN        ! Power in fertility modifier (mort_model = 2)
real(kind=kind(0.0d0)) :: st_fT        ! Power in temperature modifier (mort_model = 2)
real(kind=kind(0.0d0)) :: st_fPhys     ! Power in physmod modifier (mort_model = 2)
...
real(kind=kind(0.0d0)), allocatable :: fT_hist(:,:)    ! fT history
real(kind=kind(0.0d0)), allocatable :: fPhys_hist(:,:) ! fPhys history
integer, dimension(n_sp)            :: hist_ptr         ! circular buffer pointer per cohort
...
real(kind=kind(0.0d0)), dimension(n_sp) :: gammaF1  ! Maximum litterfall rate
real(kind=kind(0.0d0)), dimension(n_sp) :: gammaF0  ! Litterfall rate at t = 0
real(kind=kind(0.0d0)), dimension(n_sp) :: tgammaF  ! Age at which litterfall rate has median value
...
real(kind=kind(0.0d0)), dimension(n_sp) :: fCalpha700 ! Assimilation enhancement factor when CO2 = 700 ppm
real(kind=kind(0.0d0)), dimension(n_sp) :: fCg700     ! Canopy conductance enhancement factor when CO2 = 700 ppm
real(kind=kind(0.0d0)), dimension(n_sp) :: m0         ! Value of 'm' when FR = 0
real(kind=kind(0.0d0)), dimension(n_sp) :: fN0        ! Value of fN when FR = 0
real(kind=kind(0.0d0)), dimension(n_sp) :: fNn        ! Power of (1-FR) in fN
...
real(kind=kind(0.0d0)), dimension(n_sp) :: wSx1000   ! Maximum stem mass per tree @ N = 1000 trees/hectare (mort_model = 1)
real(kind=kind(0.0d0)), dimension(n_sp) :: thinPower ! Power in self-thinning rule (mort_model = 1)
real(kind=kind(0.0d0)), dimension(n_sp) :: mF        ! Mean single-tree foliage biomass of dead trees relative to mean tree
real(kind=kind(0.0d0)), dimension(n_sp) :: mR        ! Mean single-tree root biomass of dead trees relative to mean tree
real(kind=kind(0.0d0)), dimension(n_sp) :: mS        ! Mean single-tree stem biomass of dead trees relative to mean tree
...
real(kind=kind(0.0d0)), dimension(n_sp) :: gammaAPAR     ! Parameter to amplify light benefit to shorter cohorts
...
real(kind=kind(0.0d0)), dimension(n_sp) :: cVPD          ! LAI for 50% reduction of VPD in canopy
real(kind=kind(0.0d0)), dimension(n_sp) :: alphaCx       ! Maximum quantum canopy efficiency (before applying growth modifiers)
...
real(kind=kind(0.0d0)), dimension(n_sp) :: BLcond        ! Aerodynamic conductance (boundary layer conductance)
real(kind=kind(0.0d0)), dimension(n_sp) :: RGcGW         ! The ratio of diffusivities of CO2 and water vapour in air
real(kind=kind(0.0d0)), dimension(n_sp) :: D13CTissueDif ! Difference between mean annual d13C of new photosynthate and d13C of tree rings
real(kind=kind(0.0d0)), dimension(n_sp) :: aFracDiffu    ! Fractionation against 13C in diffusion through air
real(kind=kind(0.0d0)), dimension(n_sp) :: bFracRubi     ! Enzymatic fractionation by Rubisco
real(kind=kind(0.0d0)), dimension(n_sp) :: m_apar        ! PAR multiplier to redistribute unabsorbed PAR to short cohorts
...
integer, dimension(n_sp)                 :: CrownShape ! Crown shape; 1=cone, 2=ellipsoid, 3=half-ellipsoid, 4=rectangular

! Height and Volume
real(kind=kind(0.0d0)), dimension(n_sp) :: Hd                  ! Height of the diameter measurements for allometric equations
real(kind=kind(0.0d0)), dimension(n_sp) :: aH, nH1, nH2, nH3, nH4     ! Parameters of height equation
real(kind=kind(0.0d0)), dimension(n_sp) :: aV, nVB, nVH, nVBH         ! Parameters of stem volume equation
real(kind=kind(0.0d0)), dimension(n_sp) :: aK, nK1, nK2, nK3, nK4     ! Parameters of crown diameter equation
real(kind=kind(0.0d0)), dimension(n_sp) :: aHL, nHL1, nHL2, nHL3, nHL4 ! Parameters of crown ratio equation
...
real(kind=kind(0.0d0)), dimension(n_sp) :: dbh             ! Quadratic mean stem diameter at breast height, cm
...
real(kind=kind(0.0d0))                  :: height_wtav_LAI    ! LAI-weighted average height across cohorts, for height_rel_wt
...
real(kind=kind(0.0d0)), dimension(n_sp) :: biom_tree_max      ! Maximum stem mass per tree at current stocking (mort_model = 1)
...
! Modifiers ----------------------
real(kind=kind(0.0d0)), dimension(n_m, n_sp) :: f_age    ! Dimensionless growth modifier quantifying the effect of age
real(kind=kind(0.0d0)), dimension(n_m, n_sp) :: f_tmp    ! Dimensionless growth modifier quantifying the effect of temperature
real(kind=kind(0.0d0)), dimension(n_m, n_sp) :: f_tmp_gc ! Dimensionless canopy conductance modifier quantifying the effect of temperature
real(kind=kind(0.0d0)), dimension(n_m, n_sp) :: f_frost  ! Dimensionless growth modifier quantifying the effect of frost
...
real(kind=kind(0.0d0)) :: f_sw_tmp     ! temporary f_sw for rolling-average modifier calculation
real(kind=kind(0.0d0)) :: f_vpd_tmp    ! temporary f_vpd for rolling-average modifier calculation
real(kind=kind(0.0d0)) :: f_phys_tmp   ! temporary f_phys for rolling-average modifier calculation
real(kind=kind(0.0d0)) :: vpd_mean     ! mean VPD for long-term modifier calculation
...
! Temporary variables for density-dependent mortality
real(kind=kind(0.0d0))                  :: thinIntercept_eff  ! effective self-thinning intercept after modifier adjustment (mort_model = 2)
real(kind=kind(0.0d0))                  :: N_max              ! maximum stem density from self-thinning rule
real(kind=kind(0.0d0))                  :: dbh_safe           ! DBH bounded away from zero for safe division
real(kind=kind(0.0d0))                  :: expo               ! exponent in self-thinning calculation
real(kind=kind(0.0d0))                  :: logN               ! log of stem density in self-thinning calculation
real(kind=kind(0.0d0)), dimension(n_sp) :: weight             ! species weight for mortality allocation
real(kind=kind(0.0d0))                  :: weight_sum         ! sum of species weights for mortality allocation
real(kind=kind(0.0d0))                  :: loss_sum           ! total stem loss across species
real(kind=kind(0.0d0))                  :: scale              ! scaling factor for density-dependent mortality
real(kind=kind(0.0d0))                  :: pp                 ! intermediate power term (mort_model = 3)
real(kind=kind(0.0d0))                  :: dbh_prev_safe      ! previous-step DBH bounded away from zero (mort_model = 3)
...
real(kind=kind(0.0d0)), dimension(n_sp) :: biom_foliage_adj_pre_def      ! Foliage mass of trees that survived the defoliation event (adjusted for killed trees)
...
! Temporary variable for defoliation recovery
real(kind=kind(0.0d0))                  :: NPP_eff            ! effective NPP during defoliation recovery

! Temporary variables for coppice events
integer                                  :: jj                 ! loop index for coppice age recalculation
...
! Temporary variables for dbh distributions
real(kind=kind(0.0d0)), dimension(n_sp) :: dlocation          ! temporary location parameter for Weibull distribution
real(kind=kind(0.0d0)), dimension(n_sp) :: DWeibullShape_gamma ! Weibull shape parameter via gamma function
