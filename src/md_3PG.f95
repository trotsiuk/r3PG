    ...
    subroutine s_3PG_f ( siteInputs, speciesInputs, forcingInputs, managementInputs, defoliationInputs, &
       ...
       ! dbh distributions
        ...
        do ii = 2, n_m
            ...
            ! after the previous month using new NPP.
            do i = 1, n_sp
            ...
            do i = 1, n_sp
                ...
                if ( f_dormant(month, leafgrow(i), leaffall(i)) .eqv. .TRUE. ) then
                    ...
                    ! if recovering from defoliation, some additional NPP can come from non-structural carbohydrates
                    NPP_eff = NPP(i)
                           ...
                           ! calculate dominance-weighted allocation
                               do i = 1, n_sp
                                   ...
                                   if (height_rel_wt(i) > 1.d-6) then
                                       weight(i) = basal_area(i) * height_rel_wt(i)**(-1.d0) ! the 2.d0 defines the asymmetry of the weighting (also change it below)
                                   else
                                       weight(i) = 0.d0
                                   end if
                               end do
...
! Mass-conserving allocation across cohorts
                           if (mort_thinn_total > 0.d0) then
                               ...
                               do i = 1, n_sp
                                   ! protect against zero or tiny heights
                                   if (height_rel_wt(i) > 1.d-6) then
                                       weight(i) = basal_area(i) * height_rel_wt(i)**(-1.d0) ! the 2.d0 defines the asymmetry of the weighting (also change it above)
                                   else
                                       weight(i) = 0.d0
                                   end if
                               end do
